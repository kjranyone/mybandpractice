...
## ⚠️ ツール使用に関する免責事項・注意事項 (Disclaimer)

* **私的使用の範囲での利用**: 本プロジェクトに含まれる音源取得ツール (`yt-to-mp3.py`) および自動フェッチ機能は、個人またはバンド内での私的練習・学習（著作権法第30条「私的使用のための複製」）のみを目的として設計されています。
* **著作権および公衆送信の禁止**: 取得した音声データや歌詞データをインターネット上（Gitリポジトリ、Webサーバー、SNS等）に公開・アップロード・再配布する行為は厳重に禁止されます。
* **利用規約の遵守**: 対象となる動画配信サービスや各種Webサイトの利用規約（Terms of Service）を各自確認のうえ遵守してください。また、違法にアップロードされたコンテンツからのダウンロードを行わないでください。
* **自己責任**: 本ツールの使用によって生じた一切のトラブル、損害、権利侵害等について、開発者は一切の責任を負いません。
